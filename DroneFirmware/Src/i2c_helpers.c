...
HAL_StatusTypeDef i2c_transmit(I2C_HandleTypeDef* hi2c, uint16_t DevAddress, uint16_t MemAddress,
    ...
    HAL_StatusTypeDef result = HAL_I2C_Mem_Write(hi2c, DevAddress, MemAddress, MemAddSize, pData, Size, 2);
    return result;
}

HAL_StatusTypeDef i2c_receive(I2C_HandleTypeDef* hi2c, uint16_t DevAddress, uint16_t MemAddress,
                                  uint16_t MemAddSize, uint8_t* pData, uint16_t Size) {
    HAL_StatusTypeDef result = HAL_I2C_Mem_Read(hi2c, DevAddress, MemAddress, MemAddSize, pData, Size, 2);
    return result;
}
