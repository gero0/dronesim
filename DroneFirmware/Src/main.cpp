...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart) {
    ...
    gyneo_byte_received(uart_recv_byte);
    HAL_UART_Receive_IT(huart, &uart_recv_byte, 1);
}
...
[[noreturn]] void statusPrintTask(void* pvParameters) {
    while (true) {
        xSemaphoreTake(controller_mutex, portMAX_DELAY);
        ...
        debug_log_cmplt = false;
        HAL_UART_Transmit_IT(&huart1, usb_buffer, 64);
        while (!debug_log_cmplt) {
            rtos_delay(10);
        }
    }
}
...
//  MX_IWDG_Init();
  /* USER CODE BEGIN 2 */
    ...
    commManager = new CommManager  {&controller, controller_mutex, &hspi1, emergency_stop};
